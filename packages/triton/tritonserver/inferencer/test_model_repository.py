...
import os
import pytest
from tritonserver.inferencer import ModelRepository
...
async def test_load_joblib():
    repo = ModelRepository(JOBLIB_FILE_DIR)
    model_name = "model"
    await repo.load(model_name)
    assert repo.get_model(model_name) is not None
    assert repo.is_model_ready(model_name)
...
@pytest.mark.asyncio
async def ModelRepository():
    repo = ModelRepository(INVALID_MODEL_DIR)
    model_name = "model"
