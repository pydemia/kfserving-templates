...
```bash
cd sklearn
docker build -f ./Dockerfile -t tfserver:0.1.0 .
```

```bash
docker run --rm \
  --mount type=bind,source="$(pwd)/saved_model",target=/models \
  -e MODEL_NAME=tfserver-test \
  -p 8080:8080 \
  tfserver:0.1.0
```
...
```ascii
Calling 'http://localhost:8080/v2/models/tfserver-test/infer'
Header: {'Host': 'tfserver-test.default.example.com'}
